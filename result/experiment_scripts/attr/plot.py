...
if 0:
  ...
  p += axis_labels("Skew", "F1", "continuous", "continuous", 
      ykwargs=dict(lim=[.5,1], breaks=[.6, .7]),
      xkwargs=dict(breaks=[.25, .75]))
  p += theme_bw() + theme(**{
...
if 1:
  ...
  and opt_approx in (0,1,2)  and max_num_compl > 20
  """
  data = group_on([('name', 'name'), ('attr', 'num_attr'), ('301 - (logsize - corrupt_qidx)', 'corrupt_qidx')],
            where)



  if 0:
    p = ggplot(data, aes(x='num_attr', y='num_cons', color='name', shape='name', linetype='name'))
    p += geom_point(size=2) + geom_line(size=.6)
    p += facet_grid(".~corrupt_qidx", labeller="function(k,v)paste('idx:', v)")
    p += scale_color_discrete()
    p += axis_labels("# Attribute in DB (log)", "# constrainst", "log10", "continuous",
        xkwargs=dict(breaks=[10, 100, 1000], labels=map(esc, ['10', '100', '1k'])))
    p += legend_bottom
    p += guides(col = guide_legend(nrow = 2))
    ggsave("dbsize_nconstraints.png", p, libs=['grid'], width=6, height=4, scale=0.7)


    p = ggplot(data, aes(x='num_attr', y='prep_time+add_time', color='name', shape='name', linetype='name'))
    p += geom_point(size=2) + geom_line(size=.6)
    p += facet_grid(".~corrupt_qidx", labeller="function(k,v)paste('idx:', v)")
    p += scale_color_discrete()
  ...
  p = ggplot(data, aes(x='num_attr', y='total_time', color='name', shape='name', linetype='name'))
  p += geom_point(size=2) + geom_line(size=.6)
  p += facet_grid(".~corrupt_qidx") #, labeller="function(k,v)paste('idx:', v)")
  p += scale_color_discrete()
  p += axis_labels("# Attribute in DB (log)", "Time (sec)", "log10", "continuous",
      xkwargs=dict(breaks=[10, 100, 1000], labels=map(esc, ['10', '100', '1k'])))
  p += theme_bw() + theme(**{
                        "legend.position":esc("bottom"),
  ...
  p += guides(col = guide_legend(nrow = 2))
  ggsave("attr_time.png", p, libs=['grid'], width=6, height=4, scale=0.7)



  #data = fold(data, ['Precision', 'Recall', "F1"])
  #data = fold(data, [ "F1"])
  p = ggplot(data, aes(x='num_attr', y='F1', color='name', shape='name', linetype='name'))
  p += geom_point(size=2) + geom_line(size=.6)
  p += facet_grid(".~corrupt_qidx") #, labeller="function(k,v)paste('idx:', v)")
  p += scale_color_discrete()
  p += axis_labels("# Attribute in DB (log)", "F1", "log10", "continuous",
  ...
  p += guides(col = guide_legend(nrow = 2))
  ggsave("attr_acc.png", p, libs=['grid'], width=6, height=4, scale=0.7)
