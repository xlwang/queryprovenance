...
public class DatabaseState {
	...
	public DatabaseState(DatabaseHandler database, Table table,
			Complaint complaints) throws Exception {

		state = new HashMap<String, Tuple>(); // initialize state information
		this.table = table;
		String selectcondition = " true ";
		// prepare complaint condition
		if (complaints != null) {
			String[] conditions = new String[complaints.size()];
			int scpcount = 0;
			for (SingleComplaint scp : complaints.compmap.values()) {
				conditions[scpcount++] = scp.getConstraint(table);
			}
			selectcondition = Util.join(conditions, " or ");
		}
		ResultSet result;
		if (table.getName() != null) {

			// get state query prepared
			String state_query = "select * from " + table.getName() + " where "
					+ selectcondition + ";";
			...
			while (result.next()) {
				...
				for (int i = 0; i < table.getColumns().length; ++i) {
					String colname = table.getColumns()[i];
					int columnidx = column_map.get(colname);
					tuple.setValue(i, result.getString(columnidx));
				}
	...
	/* initialize database state */
	public DatabaseState(DatabaseHandler database, String tablename, Complaint complaints)
			throws Exception {
		this(database, Table.tableFromDB(database, tablename), complaints);
	}

	public void saveToDatabase(DatabaseHandler handler, String tablename)
			throws Exception {
		StringBuffer fmtsb = new StringBuffer();
		StringBuffer sb = new StringBuffer();
		sb.append(String.format("CREATE SEQUENCE %s_seq MINVALUE %d;",
				tablename, this.size() + 1));
		...
		for (int colidx = 0; colidx < table.size(); colidx++) {
			...
			if (type == Table.Type.NUM) {
				if ("id".equals(col)) {
					...
					sb.append(String
							.format("%s int primary key DEFAULT nextval('%s_seq') NOT NULL",
									col, tablename));
				} else {
					fmtsb.append("%s");
	...
	public DatabaseState getTrueState(Complaint complaint_set) throws Exception {
		...
		for (String pk : complaint_set.keySet()) {
	...
	public String getCheckQuery(String[] data) {
		...
		String checkquery = "select * from " + table.getName() + " where ";
		for (int keyidx : table.getKeyIdx()) {
			checkquery += table.getColumnName(keyidx) + " = " + data[keyidx]
					+ " and ";
		}
		checkquery += "true";
		return checkquery;
	...
	/*
	 * get value for selected feature which is composed by columns with given
	 * key list
	 */
	public String[] getFeature(String column_name, Set<String> tuple_list)
			throws Exception {
		String[] result = new String[state.size()];
		int count = 0;
		int idx = table.getColumnIdx(column_name);
