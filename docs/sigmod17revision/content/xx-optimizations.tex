...
\begin{figure}[t]
    ...
    \vspace*{-0.1in}
    \caption{Log size vs. execution time for 1000 records. \xlw{Beyond $80$ queries, the solver exceeds the $1000$sec time limit.}}
...
problem.   
\xlw{
\emph{Tuple-slicing} breaks down a large MILP problem into two, often much smaller, MILP problems and thus greatly helps improving \sys's performance, especially when the ratio of complaint set size and the database size is small. For problems with complete complaint set, by further enforcing \textbf{no non-complaint tuples} in the refinement step (disallow above zero objective-function value for the refinement MILP problem), we ensure the correctness of tuples that are not in the complaint. And thus applying \emph{tuple-slicing} is guaranteed to be correct. For problems with incomplete complaint set, \emph{basic} approach leads to infeasible MILP problems and we rely on tuple-slicing (without forcing non-zero objective value in the refinement step) to generate query log repairs. In this case, the derived query repair may introduce noises. In this paper, we mainly focus on problems with single corruption and complete complaint set, thus \emph{tuple-slicing} is an effective method to improve the performance of the \emph{basic} approach, without compromising, and often improving, repair quality.  
}
...
\xlw{This optimization only linearizes the queries in $Rel\mathcal{(Q)}$ which is a conservative estimate of all queries that may have affected the user complaints.  
...
\xlw{Similar to query slicing, this optimization simply removes irrelevant attributes through static analysis of the query log.  Thus it reduces the problem size without any loss of accuracy.}
