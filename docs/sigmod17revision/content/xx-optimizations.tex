...
\begin{figure}[t]
    ...
    \vspace*{-0.1in}
    \caption{Log size vs.\ execution time for 1000 records. \newtext{The basic approach failed to complete by the time limit of 1000sec for a log of $80$ queries.}}
...
\begin{definition}[Query dependency \& impact] \label{eq:dependency}
    ...
    \xlw{The \textbf{impact} of a query $q_i$ is the set of attributes that may updated due to $q_i$
...
\xlw{The logic behind the calculation of \textbf{full-impact} is straightforward: it traces the query history and continuously extend the impact of a query 
so that it includes every attribute that may updated by the query. Thus, the \textbf{full-impact} of a query may also extend to another query that 
has disjoint set of attributes in both \texttt{SET} and \texttt{WHERE} clauses, e.g., in a query log of $\{q_1$ writes $A$; $q_2$ reads $A$ writes $B$, $q_3$ reads $B$ writes $C\}$, 
the impact of $q_1$ extends from $\{A\}$, to $\{A,B\}$(at $q_2$) and finally $\{A, B, C\}$(at $q_3$, full-impact).}
...
\newtext{Similar to query slicing, this optimization removes only non-relevant
