...
problem.   
\xlw{
\emph{Tuple-slicing} is a \textbf{heuristic} that breaks down a large MILP problem into two, often much smaller, MILP problems. It works effectively in practice and greatly helps improving \sys's performance, especially when the ratio of complaint set size and the database size is small. However, it it possible that \emph{tuple-slicing} fixes the incorrect query and introduce noises to the query log repair. However, for problems with complete complaint set, by further enforcing \textbf{no non-complaint tuples} in the refinement step (disallow above zero objective-function value for the refinement MILP problem), we ensure the correctness of tuples that are not in the complaint. In this paper, we mainly focus on problems with single corruption and complete complaint set, under this setting, \emph{tuple-slicing} is an effective method to improve the performance of the \emph{basic} approach, without compromising, and often improving, repair quality.  
} 
