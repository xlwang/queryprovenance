...
In addition, \emph{tuple-slicing} suffers from higher mistake risk when applying to workloads 
...
However, for problems with single query corruption and complete complaint set, 
by incorporating with the \textit{incremental repair} optimization and further enforcing \textbf{no non-complaint tuples}
in the refinement step, we ensure the correctness of tuples that are not in the complaint.
