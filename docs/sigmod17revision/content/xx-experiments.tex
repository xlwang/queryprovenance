...
Note that the reported accuracy measures whether the complaint tuples are correctly repaired, and the repaired query is allowed to differ from the true query. 
However, we study the latter issue in Appendix~\ref{app:index} and find that \xlw{\sys always fixes the true query with complete complaint set, and \sys tends to 
fix wrong queries for problems with incomplete complaints where the false negative rates $>0$ on old corruption(s) with age $250$.}
...
\xlw{Finally, if there are insufficient complaints, then it is possible that the repair will over-generalize to  ``repair'' the wrong records and lead to lower precision.}
