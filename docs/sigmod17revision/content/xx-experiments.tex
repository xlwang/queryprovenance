...
\newtext{These metrics measure whether the complaint tuples are repaired correctly, but it is possible that the repaired query differs from the true query. 
...
\xlw{We now vary the database size ($N_D \in [1k, 100k]$) with a large number of attributes ($N_a = 100$).
...
we believe this increasing trend limits solver's ability to prune constraints that correspond to queries and attributes that 
...
\newtext{We find that reducing the number of reported complaints lowers the
