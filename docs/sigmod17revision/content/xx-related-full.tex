...
\xlw{Due to our assumption that the query structures are fixed, it is possible to view our problem as a data diagnosis and repair problem 
by creating a table for parameters in the query log and converting the query histories into a single nested \texttt{SELECT} query.
The data diagnosis problem focuses on detecting and summarizing source tuples that are responsible for the annotated errors in the result. Even though
there exist multiple works solving the data diagnosis problem, none of them could solve the data repair problem or address the corresponding data diagnosis problem effectively and efficiently:
~\cite{Wu13, roy2014formal} identify source tuples for aggregated \texttt{SELECT} queries only, thus could not applied to identify errors in the query log since there
is no direct mapping between an aggregate function and individual error tuples.  ~\cite{chalamalla2014} 
traces source tuples that have the highest coverage of the output errors based on their lineages. Clearly, it cannot solve the data diagnosis problem in \sys effectively 
since there is no casual relationship between data errors and their update lineage.  
~\cite{meliou2011tracing} solves the data diagnosis problem but the problem size grows exponentially with the query histories sizes, thus cannot solve the data diagnosis 
problem efficiently. 
}
...
several steps in the dataset's evolution. \xlw{Besides, other explanation works~\cite{GebalyAGKS14, Khoussainova2012, Thirumuruganathan2012, Das2011, Fabbri2011, Bender14} are also limited in their applicability as many of them target on specific application domains. }
