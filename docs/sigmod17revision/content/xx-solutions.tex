...
number $M$ is a large enough value~\cite{bazaraa2011linear} that is out of the domain of $t.A_j$, we get the
...
out of the attribute domain $M^-$ as well. In this way, subsequent conditional functions
on the attribute will return false. \xlw{Note that although the subsequent condition functions may be true initially, but any solutions having such positive conditions will have an associated objective-function value that is exceedingly large. Thus the MILP solver will always assign false to the subsequent condition functions.} So the $M^-$ assignment will not have an effect on subsequent queries encoded
in the MILP problem:
\begin{eqnarray}
\label{eq:delete}
