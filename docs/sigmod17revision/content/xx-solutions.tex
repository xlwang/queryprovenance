...
\xlw{There are nuances to how $M^-$ is set.  In particular, it cannot be set to $M$ because it biases the solver towards repairing \texttt{UPDATE} queries in lieu of \texttt{DELETE} queries (e.g., the solver repair an \texttt{UPDATE} query to set the tuple's values to $M$ rather than repair the \texttt{DELETE} query to delet the tuple).  If set to $>M$, the constraint expressions will become infeasible and the solver will fail to find a solution. Setting $M^- < M$ ensures that the generated constraint problem will be feasible and unbiased towards \texttt{UPDATE} queries. }
