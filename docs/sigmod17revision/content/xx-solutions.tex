...
\xlw{There are nuances to how $M^-$ is set.  In particular, it should be set into a large enough value such that it would not biases the solver towards repairing consecutive \texttt{UPDATE} queries in lieu of \texttt{DELETE} queries (e.g., the solver repair an \texttt{UPDATE} query to set the tuple's values to $M^-$ rather than repair the \texttt{DELETE} query to delete the tuple).  If set to $>M$, the constraint expressions will become infeasible and the solver will fail to find a solution. Setting $M^- <\leq M$ ensures that the generated constraint problem will be feasible and unbiased towards \texttt{UPDATE} queries. }
