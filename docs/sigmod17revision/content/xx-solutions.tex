...
\xlw{There are nuances to how $M^-$ is set.  In particular, for problems with incorrect \texttt{DELETE} predicate, $M^-$ should be set into a sufficiently large value such that fixing the \texttt{WHERE} clause of the \texttt{DELETE} query ($\sigma_{q_i}(t) = 0/1$) would have a much lower objective-function value than fixing the \texttt{SET} clause of a \texttt{UPDATE} query ($\mu_{q_i}(t.A_j) = M^-$).  In addition, $M^- \leq M $ since otherwise, the constraint expressions will become infeasible and the solver will fail to find a solution. }
