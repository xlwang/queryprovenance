...
\begin{definition}[Complaint]
    A complaint $c$ is a mapping between two tuples: $c: t\mapsto t^*$, such that $t$ and $t^*$ have the same schema, \newtext{$t, t^*\in D_n\cup\{\bot\}$}, and $t\neq t^*$. A complaint defines a
    transformation $\mathcal{T}_c$ on \newtext{the final database state $D_n$: $\mathcal{T}_c(D_n)
...
\xlw{Note that our framework does not require \textbf{complete} value corrections for tuples on all attributes and can be easily extended to handling problems with incomplete information. In this paper, we focus on problems with this condition and leave relaxations of this constraint to future work.}
...
In this work, we focus on handling data manipulation queries\footnote{\xlw{We avoid the term \textit{update queries} in distinguish with \texttt{UPDATE} query type.}} with simple, basic query structures without subqueries, aggregations, nor joins. \xlw{Starting with an empty or clean database, \sys supports queries with WHERE clauses containing conjunctions and disjunctions of predicates.}
...
\xlw{In addition, we target a setting where that the complaint set contains complete value corrections and are mostly derived from a single query corruption. 
We demonstrate that \sys is able to solve problems with multiple corrupt queries and incomplete value corrections, but also suffer from scalability and accuracy limitations. We leave effective and efficient techniques for solving problem in more complex settings as our future work. }
