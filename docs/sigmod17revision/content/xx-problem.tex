...
In this work, we focus on handling data manipulation \newtext{statements (\texttt{UPDATE}, \texttt{INSERT}, and
\texttt{DELETE} queries)} with simple, basic query structures without subqueries, aggregations, nor joins. \xlw{Starting with an empty or clean database, \sys supports queries with WHERE clauses containing conjunctions and disjunctions of predicates.}
