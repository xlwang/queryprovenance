...
\end{align}
We use $e_{q_i, t}$ to support query logs with \texttt{DELETE} statements.
...
If $t$ has been deleted, then $e_{q_i, t}=0$ and $x_{q_i, t}$ will simply be false, 
otherwise, $x_{q_i, t}$ depends on $t$ satisfying the condition function $\sigma_{q_i}$. 
...
$e_{q_i, t}=0$ if $t$ was deleted in query $q_i$ ($t.A_j = M^-$) and that the deletion is correct
