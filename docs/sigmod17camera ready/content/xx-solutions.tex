...
\begin{align}
\label{eq:x}
x_{q_i, t} = (\neg x_{q_i, t}') \otimes \sigma_{q_i}(t)
\end{align}
$x_{q_i, t}'$ introduced in order to support query logs with \texttt{DELETE} statements.
$x_{q_i, t}'$ is a binary indicator of whether $t$ is deleted from the database 
before executing $q_i$. If $t$ has been deleted ($x_{q_i, t}'$=1), $x_{q_i, t}$
will also be false, otherwise, $x_{q_i, t}$ is dependent $t$ satisfying the 
condition function $\sigma_{q_i}$. 
We will describe the set of $x_{q_i, t}'$ in Equation~\ref{eq:delete_x} as we introduce 
the setting of \texttt{DELETE} queries.
For simplicity, we set $x_{q_i, t}'= 0$ if there exists no \texttt{DELETE} 
...
\newtext{Using $M^-$ thus ensures that subsequent queries will treat the tuple as a \emph{``ghost''} and ignore it.}
\newtext{
\begin{eqnarray}
\label{eq:delete}
t'.A_j &=& x_{q_i, t} \otimes M^- + (1-x_{q_i, t}) \otimes t.A_j 
\end{eqnarray}
$x_{q_i, t}$ was defined in Equation~\ref{eq:x}, and 
$x_{q_i, t}'=1$ if $t$ is correctly deleted---deleted in query $q_i$ ($t.A_j = M^-$) and 
deleted in the complaint set ($t^*.A_j = M^-$).
\begin{eqnarray}
\label{eq:delete_x}
