...
\end{align}
$x_{q_i, t}'$ introduced in order to support \texttt{DELETE} statements.
$x_{q_i, t}'$ is a binary indicator of whether $t$ exists in the database 
before executing $q_i$. If $t$ has been deleted ($x_{q_i, t}'$=0), $x_{q_i, t}$
will also be false, otherwise, $x_{q_i, t}$ is dependent $t$ satisfying the 
condition function $\sigma_{q_i}$. 
For clarity, $x_{q_i, t}'=1$ for \texttt{UPDATE} and \texttt{INSERT} queries, and
we will describe to set $x_{q_i, t}'$ for \texttt{DELETE} queries below.
}
