...
We describe how to set $e_{q_i, t}$ (Equation~\ref{eq:delete_x}) when introducing \texttt{DELETE} queries.
}
...
\newtext{impacts the value of tuple $t$.}.  Each attribute of the newly inserted tuple ($t'.A_j$) may take one of two values: the value specified by the insertion query ($t_{new}.A_j$) if the query \newtext{changes the value of the tuple $t$} ($x_{q_i, t}=1$), or an undetermined value ($v.A_j$) \newtext{otherwise}.  Thus, similar with Equation~\eqref{eq:linearization}, we write:
\begin{eqnarray}
\label{eq:insert}
...
\end{eqnarray}
$e_{q_i, t}$ is set to $0$ if (1)~$t$ was deleted in $q_i$ ($t.A_j = M^-$) and (2)~the deletion is correct
