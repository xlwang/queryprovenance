...
\begin{definition}[Query dependency \& impact] \label{eq:dependency}
    ...
    condition function $\sigma_{q_i}$. \newtext{We compute query $q_i$'s impact through queries
    up to $q_{j+1}$ ($j \geq i$), denoted by $\mathcal{F}_{j+1}(q_i)$, from $q_i$'s impact to $q_j$:
    \[
    \mathcal{F}_{j+1}(q_i)=
    \begin{cases}
    \mathcal{F}_{j}(q_i) \cup \mathcal{I}(q_{j+1}),&\ if\ \mathcal{F}_{j}(q_i) \cap \mathcal{P}(q_{j+1}) \neq \emptyset \\
    \mathcal{F}_{j}(q_i),&\ otherwise
    \end{cases}
    \]
    Where $\mathcal{F}_{i}(q_i) = \mathcal{I}(q_i)$. Thus, query $q_i$'s \textbf{full impact},
    which is the set of attributes it may affect through all subsequent queries in the log, is $q_i$'s
    impact to the most recent query in the log: $\mathcal{F}(q_i) = \mathcal{F}_{n}(q_i)$.}
