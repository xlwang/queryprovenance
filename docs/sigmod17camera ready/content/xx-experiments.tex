...
\begin{figure}[t]
\centering
  \includegraphics[width = .66\columnwidth]{figures/benchmark_time}
  \vspace*{-.2in}
...
\smallskip
  ...
  \textit{Performance Limitations:
    \sys fundamentally relies on MILP solvers to produce repairs.  This comes with two scalability
    limitations evident in \sys: (1)~current solver  limitations have trouble scaling to very large problem sizes,
    and (2)~generating very large problems is memory-intensive with respect to the data and log size.  
    We use almost all memory on the experiment machines with $100$ attributes, $100k$ tuples, and $250$ queries.
