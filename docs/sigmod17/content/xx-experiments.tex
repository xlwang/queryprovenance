...
\xlw{Some thoughts about re-organization: Part1: Primary results: compare optimization and query types (Figure 7 + Figure 6c,f). Part2: Benchmark results (Figure 9). Part3: synthetic results: incremental (Figure 8); non-incremental: show multiple error results, only show QFix (Figure 6 a,d).  }
 \begin{figure*}[!htb]
    \vspace*{-.1in}
    \centering
...
\ewu{Explain why TATP has a steeper slope than TPC-C}
...
as the number of attribute or database size increases in the context of a single
corrupted query. 
...
\ewu{Need to ref Figure 6b in the text}
...
\xlw{Note that in both experiments, we omit the results of algorithms without applying tuple-slicing optimization as the these algorithms failed to scale over most of the problems. }
...
heavily depends on the property of the datasets and queries---in particular, the number of encoded candidate queries and the number of attributes. Tuple slicing is essential to solve general problems. Attribute and query slicing show significant gain for datasets with large number of attributes. }

\subsubsection{Increasing the Hardness of QueryLog}
...
These include factors such as query type, query complexity, and log size $N_q$.
...
\ewu{Probably not needed at this point.  Maybe instead say "This is why we have focused on UPDATE queries in most of our experiments.}  We observe that \texttt{UPDATE} queries translate into more undetermined variables than other query types, and are significantly more expensive to repair. Based on these results, we focus on the incremental algorithm $inc_1$ and the more difficult \texttt{UPDATE}-only workloads in the rest of the experiments.
...
\ewu{Is this setion for "solving hard problems", or examining the limitations of basic on multi-query errors?}
