...
In the following text, \sys refers to the incremental algorithm that uses \xlw{tuple-slicing optimization: we demonstrate the benefit of attribute-slicing and query-slicing optimizations on problems with $\geq 50$ attributes in the database (Section~\ref{sec:experiments:dbproperties}), however, the default \sys setting, incremental algorithm with these tuple-slicing, is sufficient in solving the vast majority of OLTP benchmarks as they seldom have more than 50 attributes.}
...
We introduce a single corruption, and vary corrupted query's index from the most recent query $q_N$ to $q_{N-1500}$\xlw{ by which $\#\ of\ Queries(N_Q)$ varies from $0$ to $1500$}.
...
By default, the tuple-slicing optimization is always enabled because none of the algorithms were able to scale without it \ewu{EXPLAIN}.
...
The database is set to the default settings ($N_a=10$, $N_D=1000$, $N_q=300$) \xlw{and we adjust the query corruption from $q_N$ to $q_{N-250}$ ($\#\ of\ Queries$ vary from $0$ to $250$). }

\stitle{Query Type: }\label{sec:indelup}
This experiment compares \qfix over \texttt{INSERT}, \texttt{DELETE}, or \texttt{UPDATE}-only query logs to test the effect of the query type \xlw{with increasing number queries from corruption to the most recent query}. 
...
\ewu{does TPCC/TATP have low dimensionality so it's not a problem?} \xlw{benchmark data are on key updates with equality condition in the predicates; they are not comparable with this experiment (range predicates).}
