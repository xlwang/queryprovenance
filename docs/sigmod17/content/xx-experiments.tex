...
and to our full technical report~\cite{qfixarxiv} for an analysis of the MILP solver, factors that affect the 
...
The database contains the default $10$ attributes and $1000$ tuples.
...
We increase the number of queries $N_q$ from $1$ to $200$ and corrupt the oldest query in the log.  
...
\ewu{Is this true:} The x-axis varies the number of queries from $1$ to $250$.
...
\ewu{does TPCC/TATP have low dimensionality so it's not a problem?}
...
The query log contains the default $300$ {\it Constant/Range} \texttt{UPDATE} queries.
...
Note that even when there are multiple corruptions in the log, the incremental algorithm is still applicable if only one is responsible for the set of complaints.}.
