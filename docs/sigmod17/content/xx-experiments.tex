...
The following two experiments compares different combinations of the slicing optimizations \emph{tuple/query/attr} under varying database size and schema size settings.  
By default, tuple-slicing is always enabled because none of the algorithms were able to scale without it \ewu{EXPLAIN}.
...
\smallskip
\textit{Takeaways: we find that repair performance is sensitive to 
the number of attributes and the number of tuples in the database, particularly when the corruption is old. 
Tuple slicing is essential to solve general problems. Attribute and query slicing show significant gain for datasets with large number of attributes. }
...
We then focus solely on the more challenging \texttt{UPDATE}-only logs and study \sys's sensitivity to query complexity and predicate dimensionality.
The database contains $10$ attributes and \ewu{XXX} tuples.
...
\ewu{Probably not needed at this point.  Maybe instead say "This is why we have focused on UPDATE queries in most of our experiments.}  
We observe that \texttt{UPDATE} queries translate into more undetermined variables than other query types, and are significantly more expensive to repair. 
\iffalse
