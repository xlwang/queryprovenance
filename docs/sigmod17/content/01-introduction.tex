...
and users execute \xlw{OLTP updates} that modify the data. 
\xlw{In fact, mistakes in such modification queries are 
frequently introduced, especially by applications that turn human inputs into 
database transactions. Thus, executing these incorrect queries may
induce additional errors to the data }and these
...
and repairs errors through query histories. \xlw{\sys 
could persists its effectiveness and efficiency with OLTP update workloads, however,
it currently cannot deal with large amounts of incorrect
information, such as fake data errors reported by malicious users. In addition, \sys 
only targets at solving common short on-line OLTP updates and it
does not support complex OLAP updates with aggregations or(and) nested sub-queries. 
}
