...
% for 
\title{{\LARGE\textbf{\sys}}: Diagnosing errors through query histories}
%%
%% Title brainstorming
%%
...
In this paper, we propose \sys, a framework that derives explanations and repairs for
discrepancies in relational data, by analyzing the effect of queries that operated on
...
known, and robust to different types of update workloads.
...
identified errors using state-of-the-art tools; (c) we present several optimizations techniques that improve our basic approach without compromising accuracy, and (d) we leverage a tradeoff between accuracy and performance to scale diagnosis to large datasets and query
logs, while achieving near-optimal results. We demonstrate the
