...
larger query logs for a database of $1000$ tuples, encoded the problem using $\sys_{exh}$ (solid line), 
...
does not over-generalize to the right because that would strictly increase the objective function.
On the other hand, over-generalizing towards the dirty range on the left is possible because it decreases
the Manhattan distance between the repaired and corrupted lower bound.

\ewu{Give second iteration a name}
