...
Specifically, 
%when $|\mathcal{F}(q) \cap \mathcal{A}(C)|=|\mathcal{A}(C)|$, $q$ may affect all complaint attributes and is a candidate for fixing; 
when $0 < |\mathcal{F}(q) \cap \mathcal{A}(C)| \le |\mathcal{A}(C)|$, 
...
We use $Rel\mathcal{(Q)}$ to denote the set of relevant queries that are candidates for repair.
