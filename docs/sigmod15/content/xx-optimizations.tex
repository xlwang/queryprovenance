...
for this important problem setting.} \alex{Is this just a note, or text meant to stay?}
...
This technique is effective for wide tables and query logs that focus a small subset of the attributes, such as TPC-C style transaction workloads.\ewu{verify claim} \alex{Should we mention here that experimentally we found that this doesn't work? I don't like that we don't understand why.}
...
Figure~\ref{fig:querysize_vs_time}, the solver runtime increases \ewu{cubically} \alex{exponentially?}
...
is parameterized, despite linearizing the same number of queries (\alex{teal?} bars).
...
\begin{algorithm}[t]
\caption{\incremental algorithm. \ewu{Should the inputs be $D_j$ or $D_0$???}
}
\label{alg:incalg}
\begin{algorithmic}[2]
...
We believe this is a natural property to expect in practice, and our experiments demonstrate \ewu{$foo\times$}
performance improvement on top of the slicing-based approaches.
