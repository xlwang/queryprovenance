...
In the case of an \texttt{INSERT} query, we additionally encode a subproblem for
the non-existant tuple $\bcancel{t}$.
The final step is to use the complaint set $\mathcal{C}$ to derive the correct values
of each tuple in the next state, and set the before and after attribute values $A_j$
...
entire query log, and how to incorporate an objective function that minimizes the amount 
of changes to the query log.
...
\subsection{Encoding the Query Log}
\label{sec:milp}
