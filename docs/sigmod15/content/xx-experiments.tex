...
in the special case when only one query has been corrupted.  We compare $\sys_{exh}$ against
...
Figure~\ref{f:singlequeryinc} shows \ewu{xxx}.
  ...
  \centering
    ...
    \end{subfigure}
    \caption{Comparing exhaustive and incremental algorithms for single query corruption.}
  \end{figure}


\stitle{Query Type Experiment:}
We now compare $\sys_{exh}$ and $\sys_{inc}$
