...
introduced  several extensions to the CPLEX-based solution~\alex{We should be careful and not tie it to CPLEX.  CPLEX is just a particular solver.  ILP-based is better.} that 
...
to study \sys in realistic scenarios. \alex{So, which algorithm do we call \sys?}
...
Each of our experiments follows a standard procedure.  \alex{what is a ``standard procedure''??}
...
2.66 GHz processor and 16GB RAM. And we use IBM CPLEX 12.6.1 to solve the 
MILP problem, WEKA J48 for as the classifier, and JAMA as the linear 
system solver. 
}
...
1000 update queries in which 530 are \texttt{UPDATE} query and the others
are \texttt{INSERT} query. 
%We then randomly perturb a subset of the queries to generate the corrupted query log. 
We corrupt the query log at certain indexes (default indexes) and for each selected query we 
replace the correct parameter(s) into randomly value within the 
...
Both workloads generated from OLTP workload 
project.\footnote{https://github.com/oltpbenchmark/oltpbench}. 
...
\subsection{MILP-based Approach vs Alternatives}
{\color{blue}
In this section, we first compare the basic MILP-based approach (Section~\ref{sec:heuristic})
with the heuristic approach (Section~\ref{sec:sol}) and demonstrate that MILP-based approach 
can solve single query effectively and efficiently compared to the heuristic
approach. We then compare the the basic MILP approach with basic MILP + optimizations 
(Section~\ref{sec:opt}) and we prove that the basic MILP approach maintain high accuracy 
in solving errors but does not scale over larger size problems. 

\smallskip 

\noindent \textbf{Heuristic Approach vs. Basic MILP-based Approach} \\
We first evaluate the performance of the MILP-based approach (Section~\ref{sec:sol}) with 
the heuristic approach, which uses decision tree
to fix the where clause and linear equations 
for the set clause (Section~\ref{sec:heuristic}). \\
\textbf{Dataset: }
Synthetic dataset with range size from [5, 10, 50, 100, 500].\\
\textbf{Performance: } ... we show heuristic approach is not as good as milp-based approach 
even on single query.  

\smallskip 

\noindent \textbf{Basic MILP-based Approach vs Optimizations} \\
We further evaluate the performance of the basic MILP-based approach with its optimizations. \\
\textbf{Comparisons: }\\
\texttt{MILP searchall}: basic MILP-based approach (MILP);\\
\texttt{MILP searchall + compl}: basic MILP-based approach with tuples in complaints only; \\
\texttt{MILP stopearly + compl}: basic MILP-based approach with tuple slicing optimization. \\
\texttt{MILP stopearly + tuple opt.}: basic MILP-based approach with all optiomizations. \\
\textbf{Dataset: } synthetic dataset with default settings.  \\
\textbf{Performance: } ... we show \texttt{MILP searchall} does not scale, 
\texttt{MILP searchall + compl} scale better but not as accurate, 
\texttt{MILP stopearly + compl} scale well but low precision, 
and \texttt{MILP stopearly + tuple opt.} scale well with high accuracy.
