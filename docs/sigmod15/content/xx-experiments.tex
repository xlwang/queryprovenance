...
We then establish the difficulty of \texttt{UPDATE} workloads as compared to other query types.
  ...
  \centering
    ...
    \end{subfigure}
    \begin{subfigure}[t]{.3\textwidth}
    \includegraphics[width = .95\columnwidth]{figures/incrementalcompare_time}
    \caption{.}
    \label{f:singlequeryinc} 
    \end{subfigure}
    \begin{subfigure}[t]{.3\textwidth}
    ...
    \end{subfigure}
    \begin{subfigure}[t]{.3\textwidth}
    \includegraphics[width = .95\columnwidth]{figures/incrementalcompare_acc}
    \caption{.}
    \label{f:singlequeryinc} 
    \end{subfigure}
    \begin{subfigure}[t]{.3\textwidth}
...
in the special case when only one query has been corrupted.  We compare $\sys_{exh,t}$ against
$\sys_{inc}^{1st}$ and $\sys_{inc}^{all}$ using the default parameters.
Figure~\ref{f:singlequeryinc} shows \ewu{xxx}.
