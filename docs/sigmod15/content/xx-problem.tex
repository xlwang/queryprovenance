...
$\mathcal{C}=\{c_1,\dots,c_k\}$, and we call it \emph{complaint set}.
...
Section\ref{???}\alex{add brief discussion somewhere}, but these
...
We now formalize the definition of the problem of diagnosing data
errors using query histories. A diagnosis is a log repair
...
complaint set is complete. We first proceed to describe a naive
approach based on decision trees that need to rely on the assumption
that $\mathcal{C}$ is complete (Section~\ref{????}).\alex{Is this
correct?}
We then proceed to describe a constraint programming formulation that
allows us to express this diagnosis problem as a mixed integer
program, and remove the completeness assumption (Section~\ref{????}).
...
(Section~\ref{????}). \alex{clean this up and add more oomph.}
