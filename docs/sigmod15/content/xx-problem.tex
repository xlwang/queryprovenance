...
database contains a single relation \ewu{with attribute $A_i,\ldots,A_m$, }
but the single table is not a requirement in our framework.\ewu{this suggests we actually handle >1 tables}
...
errors using query \ewu{logs}. A diagnosis is a log repair
...
We first outline an obvious  but flawed approach based on decision trees
that only works for query logs of size one, and complete complaint sets.
%that ass that $\mathcal{C}$ is complete (Section~\ref{????}).\alex{Is this correct?}
We then describe a constraint programming formulation that
expresses this diagnosis problem as a mixed integer linear
program (MILP), and remove the limiting assumptions in the 
decision tree approach (Section~\ref{sec:sol}).
Further, we show several optimization techniques that allow us to
scale our diagnostic framework to large data and log sizes,
including optimizations for single query case that lets us scale by $y\times$ 
beyond the basic MILP approach
(Section~\ref{sec:opt}). \alex{clean this up and add more oomph.}
