...
\xlw{
In this paper, we assume queries are standard queries with no sub-query, aggregation and join. 
More specifically, we support queries with linear SET/WHERE clauses in order to transform them as
linear constraints. There can be multiple set expressions in the SET clause or multiple where predicates 
in \textit{conjunction/disjunction} format in the WHERE clause. In Section~\ref{sec:experiments:synth}, 
we evaluate \sys in fixing \texttt{UPDATE} query with adaptable predicate sizes. 
