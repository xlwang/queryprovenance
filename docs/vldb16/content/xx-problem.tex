...
\ewu{
In this paper, we assume basic DML queries with no sub-queries, aggregations nor joins. 
We support SET expressions that set an attribute to the linear combination of attributes
WHERE clauses can be composed of conjunctions and disjunctions of predicates, where predicates
can compare linear combinations of attributes.
Section~\ref{sec:experiments:synth} evaluates \sys while varying the number of predicates in the WHERE clause.
}

% Specifically, we support queries with linear SET/WHERE clauses in order to transform them as
% linear constraints. There can be multiple set expressions in the SET clause or multiple where predicates 
% in \textit{conjunction/disjunction} format in the WHERE clause. 
...
handle $10\times$ \ewu{more what?} compared to the basic MILP approach. 

Due to space considerations, we leave out a discussion of alternative approaches that use e.g., classification tools and linear systems of equations.
...
% In the Appendix A of ~\cite{qfixarxiv}, we provide detailed description and experimental comparison 
