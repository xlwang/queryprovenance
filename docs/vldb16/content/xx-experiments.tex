...
The schema contains $N_a=10$ attributes $a_1\ldots a_10$, whose values are
...
We corrupt up to -1500 queries to the most recent query.  
}
...
In this experiment, we compare the exhaustive algorithm (no-opt) against 
each slicing optimization individually($tuple, attr, query, all$), and all of them together with four types of algorithms. 
...
incremental without tuple slicing (basic, $no-opt$) against with tuple slicing at different batching levels of 1, 2, 8 ($1,2,8$). 
...
In addition, Figure~\ref{f:singlequeryinc_acc} also shows a significant drop in accuracy as we increase the incremental level since 
the failure rate of the MILP problem is positive proportional to the incremental level.}
...
$\sys$ on insert, delete, or update-only workloads.
...
query types. \xlw{Based on the result, we consider incremental
...
Figure~\ref{f:benchmark} plots the performance of the incremental and optimized algorithm ($\sys$)
...
{\it Takeaway: many workloads in practice are dominated by point update queries.  In these settings,
 ...
 interactive latencies.}
  ...
  \begin{figure*}[h]
    ...
    \caption{Database Size vs. Time (\# of Attr. = 100)}
    \label{f:attr100} 
    \end{subfigure}
    \caption{Comparison of Optimizations. \xlw{first exp need to rerun. }}
  \end{figure*}
\iffalse  
  \begin{figure*}[h]
    \centering
...
in the table from $10$ to $100, 1k$ with $N_D = 1k$. As shown 
...
Overall, $\sys_{taq,inc-1}$ saves more than $xxx$ cost when the number of attribute is above
$xxx$.} \\
...
$100$ to $500, 1k, 5k$ while keep $N_a = 100$. As shown in Figure~\ref{f:attr100}, $\sys_{taq,inc-1}$ 
shows constant percentage of gain as the database size increases. 
} 
...
database size, from $100$ to $200$ seconds.   Despite this, the F1-score remains near perfect (the average F1 score is above 0.993).
...
We also observe that \texttt{UPDATE}-only workloads with constant update is 
easier to solve than relative update. There are two reasons: 1. constraints we constructed for the constant update are simpler than relative update due to the set clause structure; 2. the size of complaints of constant updates is oftentimes smaller than the relative updates since the affected tuples of the corrupt query always expands in the latter case. \\
We omit the accuracy result as \sys achieves near perfect F1-scores in all settings (the average F1 score is above 0.998). 
}
...
Figure~\ref{f:skew_time} shows that the performance is weakly related to the skewness. \xlw{add explanations.}
