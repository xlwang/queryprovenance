...
The tuple-slicing scales significantly better (up to \ewu{INSERT A NUMBER}$\times$ faster), however
batching above $k=1$ severely degrades the \ewu{REPAIR ACCURACY OR INFEASIBILITY RATE AND WHY??? "failure rate" is ambiguous.}.
...
We first evaluate \sys and its optimizations by increasing the number of attribute ($N_a \in [10, 500]$) with $N_D = 100$\ewu{100 or 1000?}. 
As shown in Figure~\ref{f:attr}, when the number of attribute in a table is small (e.g., 10) all algorithms appear identical. 
However, increasing the number of attribute exhibits a larger benefit for query and attribute slicing (\ewu{Mention avg reduction in number of constraints in the problem compared to tuple only}).
...
We also vary $V_d$ in order to ensure that the cardinality of each query remains constant as the database increases in size.
This is used to ensure the size of the complaint set does not affect the experimental results.
...
In fact, combining all three optimizations improves the latency by $2\times$ in the worst case, and up to $4\times$ in the best.
\xlw{
