...
In this experiment, we evaluate the efficacy of the naive and 
incremental repair algorithm (Section~\ref{sec:incremental})
...
{\it Takeaway: many workloads in practice are dominated by point update queries.  In these settings,
 ...
 interactive latencies.}
  ...
  \begin{figure*}[h]
    ...
    \end{subfigure}
    \vspace*{-.1in}
    \caption{Comparison of Optimizations. \xlw{first graph partially done. }}
  \end{figure*}
\iffalse  
  \begin{figure*}[h]
    \centering
...
easier to solve than relative update. There are two reasons: 1. constraints we constructed for the constant update are simpler than relative update due to the set clause structure; 2. the size of complaints in constant updates is oftentimes smaller than relative updates as the affected tuples always expands in the latter case. 
...
while keeping the overall selectivity constant.  \xlw{ NEED TO REWRITE AFTER experiment done. 
...
% space in the MILP problem.
  ...
  \begin{figure*}[h]
	...
	\begin{subfigure}[t]{.3\textwidth}
      \includegraphics[width = .99\columnwidth]{figures/where_time}
      \vspace*{-.2in}
      \caption{Query Complexity (Where Size) \xlw{not done yet}.}
      \label{f:where_time} 
    \end{subfigure}
    \begin{subfigure}[t]{.3\textwidth}
