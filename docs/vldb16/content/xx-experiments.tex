...
Our reported metrics are the average across multiple\ewu{How many??} runs.
...
picked from \ewu{what is Vd??} $V_d$ uniformly at random, along with a primary key $id$.
...
workloads as compared to other query types. \ewu{The following sentence is wishy-washy.  What is it trying to say?}
...
\incremental without tuple slicing ($no-opt$ \ewu{formatted really weird, call it noopt, or base or something else}) against tuple slicing at 
different batching levels of 1, 2, 8 ($1,2,8$) \ewu{batching has not been introduced at all.  Need to describe in optimizations section and remind reader what it is here}. 
...
In addition, Figure~\ref{f:singlequeryinc_acc} also shows a significant drop in accuracy as we increase the incremental level \ewu{here you use "incremental level" instead of "batching level".  Be consistent} from $1$ 
...
Figure~\ref{f:tpcctatp} plots the performance of the incremental and \ewu{how optimized?  tuple slicing or all??} optimized algorithm ($\sys$)
...
\subsection{Synthetic \ewu{Incremental?} Experiments}
\label{sec:experiments:synth}
We now perform a detailed evaluation of the \ewu{is this correct?} incremental algorithm.
