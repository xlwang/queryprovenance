...
\ewu{Due to the difficulty of collecting corrupt query logs from active deployments, our goal instead} is to understand these trade-offs in
...
We generate and corrupt a query log as described below. 
...
Finally, we execute the algorithms and compare \ewu{the fixed query log with the true query log}, as well as the fixed and true
...
\ewu{Finally, we evaluated \sys on Example~\ref{ex:taxes} in Figure~\ref{fig:example} and fully repaired the correct query within milliseconds.}

{\it Takeaway: many workloads in practice are dominated by \texttt{INSERT} and point \texttt{UPDATE} queries (ignoring the dominant percentage of read-only queries).  
