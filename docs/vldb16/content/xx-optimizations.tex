...
\ewu{This increase has a large impact on the running time of the solver, since it needs to find a (near)-optimal 
...
\ewu{Wasn't clear from your text what we do -- this is my understanding.}
\ewu{
  ...
  In this way, we {\it refine} the solutions from the previous step while incorporating knowledge about complaints in $\mathcal{NC}$, 
  Finally, we use a new objective function that minimizes the number of non-complaint tuples 
