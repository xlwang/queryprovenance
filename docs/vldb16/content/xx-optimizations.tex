...
\xlw{
We create a new MILP using $\mathcal{C}^+$ as the complaint set, and fix binary variable $x_{q,t}$ for complaint tuples $t \in \mathcal{C}$ and the values of all queries not in $\mathcal{Q}^*_{rep}$ to their assignment in the solution from the first step.
...
problem, making \emph{tuple-slicing} optimization an effective method to improve the performance of the \emph{basic} approach, without compromising accuracy.
...
\begin{algorithm}[t]
\caption{$Inc:$ The incremental algorithm. 
}
\scriptsize
\label{alg:incalg}
\begin{algorithmic}[2]
...
Our \emph{$inc_k$} approach, described as $Inc$ in Algorithm~\ref{alg:incalg}, focuses on the case where there is a single corrupted query to repair.
It does so by linearizing the full query log, including any slicing optimizations, but only parameterizes and repairs queries in batches such that there's only $k$ undetermined queries at a time. We call $k$ as the batching level in the incremental algorithm. 
This procedure is applied from the $k$ most recent queries, attempts to repair them, and continues to the next $k$ most recent query if a repair is not generated.
The algorithm internally calls a modified version of \emph{basic} approach that takes extra parameters $\{q_i, q_{i+k}\}$.  
The modified \emph{basic} approach only parameterizes $\{q_i, q_{i+k}\}$, and fixes the values for all other parameters. % the parameters for the rest of the queries in $\mathcal{Q}$.
...
Our experiments further evaluate different batching level $k$ in the incremental algorithm and show that it is impractical from both a performance and accuracy to have $k$ greater than 1 in fixing single error problems.}
