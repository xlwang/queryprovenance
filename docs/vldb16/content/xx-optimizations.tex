...
This is depicted in the red bars in Figure~\ref{fig:querysize_vs_time} for increasing
the query log size over a database of $1000$ tuples.  The red bars encode the problem using  the \emph{basic} algorithm that parameterizes all queries, while the blue bars show the potential gain of  parameterizing a single (presumed incorrect) query.
At a log size of $80$, the solver for \emph{basic} failed to produce an answer within $1000$ seconds.
Although empirical solver performance can vary significantly, 
