...
updates (\alex{I don't understand this} \xlw{one reviewer complained `` Integrity constraints can do more than specify ranges.'' }), 
...
dataset, \ewu{and allow an administrator to easily identify and further validate the most likely query-based sources of these errors. 
...
the data that would have otherwise remained undetected.  \ewu{Consider the following motivating examples, the first is based on conversations with a large US-based wireless provider:}

\begin{example}[Wireless discount policies]\label{ex:telco}

A \ewu{\sout{large US-based}} wireless provider offers company discounts as incentives for
...
those queries.  \ewu{Once these queries have been validated (say, by an expert), they can be used to} identify unreported
...
\begin{itemize}[leftmargin=*, topsep=0mm, itemsep=0mm]      
    ...
    \item We \ewu{perform a thorough evaluation of the trade-offs between speed and accuracy of our baseline and optimized methods under a controlled, synthetic setting. \xlw{In particular, we demonstrate that the optimized \sys achieves 40$\times$ speedup compared with baseline algorithm when attribute size in the table is $500$ (Section~\ref{sec:experiments}). }
...
\ewu{These challenges present exciting future extensions to the system presented in this work.}
