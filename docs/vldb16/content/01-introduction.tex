...
\xlw{Review1 complained about these citations. } {\color{red}
...
\ewu{Instead of talking about data cleaning and transhing it, maybe focus on the value of having explanations of the errors.  Maybe something like:
1) analysis is hard and errors make it harder 2) could deal with it by preventing any errors, but that's hard.  3) lots of good work on finding errors as the result of analysis, or anomaly detection. 
4) where did these errors come from?  Want to be able to find the source. The ability to do so could be used to explain surprising errors/results in the database, or identify new errors.
5) There is a general \emph{Explanation} problem of deriving useful explanations of errors to users.
6) Within this general explanation problem, we focus on databaes and queries. Call this query explanation
7) Query explanation is hard because of X Y Z }
...
updates (\xlw{for ordered attributes}), 
...
dataset, \xlw{\emph{fix} errors from source, 
and also potentially lead to the identification of additional discrepancies in
...
\xlw{In these application scenarios, data errors are typically reported to a
...
The goal of \sys is to identify the query or queries that caused
the errors, propose corrections to those queries, and use the modified queries to
prevent introducing more errors and identify unreported errors.
This problem has the following important characteristics that render 
traditional base-by-case fixing \sout{traditional data cleaning} 
unsuitable:}
...
\end{description}
% 
\xlw{In fact, \sys is different with and serves as a complement of
traditional data cleaning methods.
Instead of identifying errors, 
\sys targets on diagnosing, explaining, and fixing errors from the root by
leveraging reported or detected errors that acquired from 
...
\begin{itemize}[leftmargin=*, topsep=0mm, itemsep=0mm]      
    ...
    methods against \sout{real-world} benchmark and synthetic datasets and query logs. 
    (Section~\ref{sec:experiments}). 
\end{itemize}

\xlw{
\sys is the first system (to our knowledge) that tries to diagnose and fix errors from query
history. It solves standard types of queries that could be found in most of the common benchmarks 
effectively and efficiently when enough accurate information is provided. However, \sys may fail 
in some complex scenarios when there are hard queries (e.g, queries with sub-query, join, and aggregation), 
or inaccurate reported errors (e.g., false positives). 
}
